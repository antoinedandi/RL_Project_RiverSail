...
class UCRL3:
    def __init__(self, nS, nA, delta, K=-1):
        ...
        if (K == -1):
            K = nS
        self.deltaSA = delta / (nS * nA * (
                    3 + 3 * K))  # 3 for rewards (1 hoeffding, 2 empBernstein), nS for peeling, nS for Berend up, nS for Beredn down
        self.observations = [[], [], []]
    ...
    def q_upper(self, pest, n, delta):
        q = search_up(lambda x: x - pest <= min(self.confbound_BernoulliBernsteinPeeling(x, n, delta),
                      ...
                      epsilon=0.0001)
        return q
    # In case one wants to use local KL ioptimization instead; (self.deltaSA needs to be modified accordingly)
    # elln = self.elln(n, delta)
